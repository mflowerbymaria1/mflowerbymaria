...
function ProductosContent() {
    ...
    const getCategoryTitle = (catSlug) => {
        ...
        switch (catSlug.toLowerCase()) {
            ...
            case 'para-sumar-a-tu-carrito':
            case 'articulos-para-sumar-a-tu-carrito':
            case 'articulos-de-libreria-para-llevar': return "Para sumar a tu carrito";
    ...
    const getCategoryDescription = (catSlug) => {
        ...
        switch (catSlug.toLowerCase()) {
            ...
            case 'para-sumar-a-tu-carrito':
            case 'articulos-para-sumar-a-tu-carrito':
            case 'articulos-de-libreria-para-llevar': return "Los elegidos de librería que no te pueden faltar para llenar tu cartuchera y tu escritorio de ternura y color.";
    ...
    const filteredProducts = allProducts.filter(p => {
        if (categoria) {
            ...
            if (matchedCategory) {
                // A product belongs to the category if its category string exactly matches the DB category name
                // OR if the product's category slug matches the DB category slug (handles legacy products like 'Cuadernos A5')
                const isExactMatch = p.category === matchedCategory.name;
                const isSlugMatch = getSlug(p.category) === matchedCategory.slug;
                
                if (!isExactMatch && !isSlugMatch) return false;
            } else {
    ...
    useEffect(() => {
        try {
            ...
            if (wholesaleSession) {
                setIsWholesale(true);
                const parsed = JSON.parse(wholesaleSession);
